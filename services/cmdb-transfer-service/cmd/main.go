...
import (
	...
	"cmdb-v2/services/cmdb-transfer-service/internal/router"
	"log"
	"os"
...
func main() {
	if err := logging.Init("cmdb-transfer-service"); err != nil {
	...
	r := gin.Default()
	router.Init(r, h, database)
	port := os.Getenv("PORT")
	if port == "" {
		port = "8080"
	}
	log.Printf("cmdb-transfer-service starting on :%s", port)
