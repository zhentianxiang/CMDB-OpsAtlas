...
onMounted(() => {
  useResizeObserver(contentRef, async () => {
    await nextTick();
    delay(60).then(() => {
      if (tableRef.value?.getTableDoms()) {
        ...
        <template v-slot="{ size, dynamicColumns }">
          ...
          >
            <template #operation="{ row }">
              ...
              <el-button
                v-if="hasPerms('sys:role:assign_menu')"
                class="reset-margin"
                link
                type="primary"
                :size="size"
